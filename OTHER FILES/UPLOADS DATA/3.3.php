...
while ($row = mysqli_fetch_array($res)) {
    ...
    $image = imagecreatefrompng("3.3.png");
    ...
    $educa = "EDUCATION";
    imagettftext($image, 60, 0, 320, 1450, $colorA, $font, $educa);

    $educaAB = $row['yearA'];
    imagettftext($image, 45, 0, 320, 1580, $colorA, $font, $educaAB);

    $educaAA = $row['eduA'];
    imagettftext($image, 45, 0, 320, 1680, $colorA, $font, $educaAA);

    $educaBB = $row['yearB'];
    imagettftext($image, 45, 0, 320, 1810, $colorA, $font, $educaBB);

    $educaBA = $row['eduB'];
    imagettftext($image, 45, 0, 320, 1910, $colorA, $font, $educaBA);



    $langu = "LANGUAGES";
    imagettftext($image, 60, 0, 320, 2120, $colorA, $font, $langu);

    $languA = $row['lA'];
    imagettftext($image, 45, 0, 320, 2260, $colorA, $font, $languA);

    $languB = $row['lB'];
    imagettftext($image, 45, 0, 320, 2390, $colorA, $font, $languB);

    $languC = $row['lC'];
    imagettftext($image, 45, 0, 320, 2520, $colorA, $font, $languC);



    $skill = "SKILL";
    imagettftext($image, 60, 0, 320, 2770, $colorA, $font, $skill);

    $skillA = $row['skillA'];
    imagettftext($image, 45, 0, 320, 2900, $colorA, $font, $skillA);

    $skillB = $row['skillB'];
    imagettftext($image, 45, 0, 320, 3030, $colorA, $font, $skillB);

    $skillC = $row['skillC'];
    imagettftext($image, 45, 0, 320, 3160, $colorA, $font, $skillC);

    $skillD = $row['skillD'];
    imagettftext($image, 45, 0, 320, 3290, $colorA, $font, $skillD);

    $skillE = $row['skillE'];
    imagettftext($image, 45, 0, 320, 3420, $colorA, $font, $skillE);
